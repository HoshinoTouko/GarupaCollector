...
class Data:
    ...
    @staticmethod
    def data_filter(data, page=1, num=20):
        """
        Sort and filter data by page and num.
        :param data:    Data fo filtered
        :param page:    Page number
        :param num:     Number of items per page
        :return:        Result, current page, total size, number of page
        """
        data.reverse()
        size = len(data)
        if size < (page-1)*num or num == 0:
            return [], page, size, 0
        else:
            result = sorted(data, key=itemgetter('time'), reverse=True)
            start = (page-1) * num
            end = start + num if start + num < size else size
            result = result[start:end]
        return result, page, size, int(size/num) + 1
